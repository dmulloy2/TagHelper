...
import com.comphenix.taghelper.handler.LegacyPacketHandler;
import com.comphenix.taghelper.handler.TagPacketHandler;
...
public class TagHelperMod extends JavaPlugin {
	...
	public void onEnable() {
		...
		if (MinecraftVersion.getCurrentVersion().isAtLeast(MinecraftVersion.BOUNTIFUL_UPDATE)) {
			manager.addPacketListener(new TagPacketHandler(this, getServer().getPluginManager()));
		} else {
			manager.addPacketListener(new LegacyPacketHandler(this, getServer().getPluginManager(), logger));
		}
	}
